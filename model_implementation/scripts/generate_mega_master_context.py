...
def create_mega_context():
    content = r"""# BLOCKTRAIN: The Mega Master Technical Blueprint & System Context
...
"""
    # Write to workspace
    ws_file = "model_implementation/BLOCKTRAIN_MASTER_PROJECT_CONTEXT.md"
    with open(ws_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[SUCCESS] Wrote workspace file: {os.path.abspath(ws_file)}")

    # Write to artifact folder
    art_dir = r"C:\Users\ADMIN\.gemini\antigravity\brain\4396eaac-5d78-4ec3-aa25-935514f1cc27"
    os.makedirs(art_dir, exist_ok=True)
    art_file = os.path.join(art_dir, "BLOCKTRAIN_MASTER_PROJECT_CONTEXT.md")
    with open(art_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[SUCCESS] Wrote artifact file: {os.path.abspath(art_file)}")
