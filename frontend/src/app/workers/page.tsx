...
export default function WorkersPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col selection:bg-indigo-600/30">
      <EnterpriseHeader badgeText="Gang Logistics • Live Duty" />

      {/* Subheader */}
      <div className="border-b border-slate-200 bg-white/80 backdrop-blur-md px-6 py-4">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Add Worker Section */}
        <div className="bg-white/80 border border-slate-200 p-6 rounded-2xl shadow-lg backdrop-blur-md">
          ...
          <form onSubmit={handleAddWorker} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <select 
                ...
                className="w-full bg-white border border-slate-300 rounded-xl text-slate-900 px-3.5 py-2.5 text-xs focus:outline-none focus:border-indigo-500 shadow-sm transition-colors"
              >
            ...
            <div className="flex items-end">
              <button 
                type="submit"
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm font-bold text-xs py-2.5 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-1.5"
              >
                Enroll Supervisor
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {DEPARTMENTS.map(dept => {
            ...
            return (
              <div key={dept} className="bg-white/70 border border-slate-200 rounded-2xl p-5 flex flex-col shadow-md">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[420px]">
                  {deptWorkers.length === 0 ? (
                    ...
                    deptWorkers.map(w => (
                      <div
                        key={w.id}
                        className="bg-slate-50/80 border border-slate-200 hover:border-slate-200 p-3.5 rounded-xl flex items-center justify-between group transition-all"
                      >
